...
def LHSPointSelection(NumPoints=15, ParameterRanges=[dict(Min=0, Max=1), dict(Min=0, Max=1)]):
    NumDimensions = 2
    ...
    Points[:, 0] = ParameterRanges[0]['Min'] + (ParameterRanges[0]['Max'] - ParameterRanges[0]['Min']) * (
                edges[np.random.permutation(NumPoints)] + step * np.random.uniform(low=ParameterRanges[0]['Min'], high=ParameterRanges[0]['Max'], size=NumPoints))

    for i in range(1, NumDimensions):
        Points[:, i] = ParameterRanges[i]['Min'] + (ParameterRanges[i]['Max'] - ParameterRanges[i]['Min']) * (
                    edges[np.random.permutation(NumPoints)] + step * np.random.uniform(low=ParameterRanges[i]['Min'], high=ParameterRanges[i]['Max'], size=NumPoints))
...
if __name__ == "__main__":
    import pandas as pd
    Points = LHSPointSelection(5, [dict(Min=0, Max=1), dict(Min=0, Max=1)])
    print(Points)
