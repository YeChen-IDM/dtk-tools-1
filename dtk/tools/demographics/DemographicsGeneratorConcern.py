...
class FnDemographicsGeneratorConcern(DemographicsGeneratorConcern):
    ...
    def __init__(self, update_node_fn: Optional[Callable[[dict, Node, dict, dict], None]] = None,
        ...
        if all([x is None for x in [update_node_fn, update_defaults_fn]]):
            raise ValueError("FnDemographicsGeneratorConcern requires at either update_node_fn or update_defaults_fn to"
                             " be a function")
        super().__init__()
    ...
    def update_node(self, defaults: dict, node: Node, node_attributes: dict, node_individual_attributes: dict):
        if self.update_node_fn:
            self.update_node_fn(defaults, node, node_attributes, node_individual_attributes, **self.custom_args)
        pass
...
def demographics_generator_node_concern(update_defaults_fn: Optional[Callable[[dict], dict]] = None,
                                        **custom_args) -> Callable:
    """
    ...
    def dg_builder_decorator(func: Callable[[dict, Node, dict, dict], None]) -> FnDemographicsGeneratorConcern:
...
class DemographicsGeneratorConcernChain(DemographicsGeneratorConcern):
    ...
    @staticmethod
    def from_list(concerns: List[DemographicsGeneratorConcern]) -> 'DemographicsGeneratorConcernChain':
...
class DefaultsDictionaryGeneratorConcern(DemographicsGeneratorConcern):
    ...
    def __init__(self, individual_attributes: dict, default_node_attributes: Optional[dict] = None,
                 ...
                 update_node: Optional[
                     Callable[[DemographicsGeneratorConcern, dict, Node, dict, dict], None]] = None,
                 ):
        """
...
class DefaultWorldBankEquilibriumConcern(DemographicsGeneratorConcernChain):
    def __init__(self, prevalence_flag: int = distribution_types["UNIFORM_DISTRIBUTION"], prevalence1: float = 0.13,
        ...
        """
        wb_concern = WorldBankBirthRateConcern(country=country, birthrate_year=birthrate_year)
