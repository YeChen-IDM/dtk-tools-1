...
def get_argument_parser() -> ArgumentParser:
    ...
    parser.add_argument('compiled_demographics_file', type=argparse.FileType('r'),
                        help='Path to a compiled demographics file')
    parser.add_argument('migration_file', type=MigrationTypes, choices=list(MigrationTypes))
    parser.add_argument('migration_type', type=MigrationTypes, choices=list(MigrationTypes))
    return parser
...
def get_migration_json(demogjson, maxvalcount, mig_type, tool):
    ...
    for node in demogjson['Nodes']:
        if mig_type != "sea" or (
                strmap['Seaport'] in node[strmap['NodeAttributes']] and
                node[strmap['NodeAttributes']][strmap['Seaport']] == 1
        ):
            migoffsets += demogoffsets[nodecount * 16: nodecount * 16 + 8] + '%0.8X' % (
                    nodecountfound * maxvalcount * 12)  # 12 -> sizeof(uint32_t) + sizeof(double)
            nodecountfound += 1
        nodecount += 1
...
if __name__ == "__main__":
    ...
    main(sys.argv[0], opts.compiled_demographics_file, opts.migration_file, opts.migration_type)
